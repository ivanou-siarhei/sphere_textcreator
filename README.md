...
запрос на текст (через чат кошелька или через textarea на лендинге), платит `PRICE` UCT
на неймтег агента, агент генерирует текст через OpenRouter LLM и присылает его в DM.
...
Сопоставление оплаты с заявкой (`findPendingForTransfer` в `agent/index.js`) проверяет
входящий перевод в следующем порядке:

1. `senderPubkey` отправителя — совпадение с ключом заявки;
2. memo перевода = код заказа `TC-XXXXXX`;
3. совпадение `senderNametag`.

Если сумма перевода меньше, чем `PRICE`, заявка остаётся в ожидании, а пользователь
получает сообщение с недостающей суммой.

## Отправка запроса через лендинг

На лендинге есть textarea для запроса. Запрос отправляется агенту через Sphere Connect
`dm` intent (`{ to, message }`), т.е. попадает в тот же обработчик `onDirectMessage`,
что и запросы из чата кошелька. Если кошелёк ещё не подключён, страница сначала запускает
подключение, а затем отправляет запрос. После получения от агента ответа с кодом
`TC-XXXXXX` пользователь вводит его в поле «Order code» — значение передаётся в memo
перевода, благодаря чему оплата находит заявку по шагу 2 из схемы выше.
...
Все тексты лендинга на английском. На странице есть textarea для отправки запроса агенту
(через `dm` intent), подключение кошелька Unicity Sphere через Sphere Connect Protocol
(`autoConnect`, popup открывается на `sphere.unicity.network`) и кнопка оплаты `PRICE` UCT
через `send` intent — сайт не касается ключей пользователя.
